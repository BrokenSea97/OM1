...
class SerialMotor(ActionConnector[SerialMotorConfig, MotorInput]):
    def __init__(self, config: SerialMotorConfig):
        ...
        self.ser = None
        logging.info(
            f"SerialMotor initialized in lazy mode. Target: {self.config.port}"
        )
    ...
    async def connect(self, output_interface: MotorInput) -> None:
        ...
        signal_map = {
            MotorAction.FORWARD.value: "F",
            MotorAction.BACK.value: "B",
            MotorAction.STOP.value: "S",
            MotorAction.LEFT.value: "L",
            MotorAction.RIGHT.value: "R",
        }
